...
The `sketchmatter-demo-vault/` directory is a ready-to-use Obsidian vault. When adding new plugin features:

- Add representative sample notes to `sketchmatter-demo-vault/` demonstrating the feature.
- Keep sample notes small and clearly named (e.g. `biome-highland.md`, `river-azul.md`).
- Update `sketchmatter-demo-vault/README.md` if new files require explanation.
- Do **not** commit `sketchmatter-demo-vault/.obsidian/plugins/` build artifacts — those are regenerated by `scripts/build-to-demo-vault.ps1`.
...
- **Adding a shape?** Follow the pattern in `src/shapes/polygon.ts`, register it in `src/shapes/index.ts`, and add a sample note to `sketchmatter-demo-vault/`.
